    ...
    public static class DataService
    {
        ...
        public static async Task<ItemsResponse<VkAudioAlbum>> GetUserAlbums(long ownerId = 0, int count = 0, int offset = 0)
        {
            ...
            catch (VkTokenConfirmationRequired)
            {
                ...
                return await GetUserAlbums(ownerId == 0 ? _vkontakte.AccessToken.UserId : ownerId, count, offset);
            }
        ...
        public static async Task<ItemsResponse<VkAudio>> GetUserTracks(int count = 0, int offset = 0, long albumId = 0, long ownerId = 0)
        {
            try
            {
                ...
                var response = await _vkontakte.Audio.Get(ownerId == 0 ? _vkontakte.AccessToken.UserId : ownerId, albumId, count, offset);
                if (response.Items != null)
                {
                    ...
                    foreach (var vkNewsEntry in vkNews.Items)
                    {
                        ...
                        if (attachments != null)
                        {
                            var audioEntries = (from a in attachments
                                                ...
                                                select a.Owner_Id + "_" + a.Id).ToList();
                            if (audioEntries.Any())
        ...
        public static async Task<bool> SetMusicStatus(VkAudio audio, string targetIds = null)
        {
            if (audio == null)
                return false;
            
            var result = await _vkontakte.Audio.SetBroadcast($"{audio.Owner_Id}_{audio.Aid}", _vkontakte.AccessToken.UserId.ToString());
            return result != null;
        }
        ...
        public static async Task<bool> AddAudio(VkAudio audio, string captchaSid = null, string captchaKey = null)
        {
            if (string.IsNullOrEmpty(audio.Source))
            {
                ...
                if (vkAudio != null)
                {
                    ...
                    audio.Owner_Id = vkAudio.Owner_Id;
                    audio.AlbumId = vkAudio.AlbumId;
                    audio.LyricsId = vkAudio.LyricsId;
                }
            }

            var newId = await _vkontakte.Audio.Add(long.Parse(audio.Id), audio.Owner_Id, captchaSid: captchaSid, captchaKey: captchaKey);
            if (newId > 0)
            {
                audio.Id = newId.ToString();
                audio.Owner_Id = _vkontakte.AccessToken.UserId;
        ...
        {
            var result = await _vkontakte.Audio.Delete(long.Parse(audio.Id), audio.Owner_Id);
            if (result)
            {
        ...
        public static async Task CopyAlbum(string title, long albumId, long ownerId)
        {
            ...
            if (audio.Items != null && audio.Items.Count > 0)
            {
                ...
                for (int i = 0; i < audio.Items.Count; i++)
                {
                    ...
                    try
                    {
                        var newAudioId = await ViewModelLocator.Vkontakte.Audio.Add(long.Parse(track.Id), track.Owner_Id, captchaSid: captchaSid, captchaKey: captchaKey);
