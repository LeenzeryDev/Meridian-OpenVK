            ...
            if ((bool)result)
            {
                try
                {
                    ...
                    for (var i = audios.Count - 1; i > 0; i--)
                    {
                        ...
                        try
                        {
                            var newAudioId = await ViewModelLocator.Vkontakte.Audio.Add(long.Parse(vkAudio.Id), vkAudio.Owner_Id, captchaSid: captchaSid, captchaKey: captchaKey);
                            if (newAudioId != 0)
                            {
